...
@Composable private fun Create(done: (String) -> Unit, back: () -> Unit) { var name by remember { mutableStateOf("MinhaRede") }; Title("Criar rede", back); OutlinedTextField(name, { name = it }, label = { Text("Nome da rede") }, Modifier.fillMaxWidth()); Spacer(Modifier.height(12.dp)); Text("Um ID e uma senha forte serão gerados para compartilhar com seus amigos.", color = Color.LightGray); Spacer(Modifier.height(18.dp)); Button({ done(name) }, Modifier.fillMaxWidth()) { Text("CRIAR E CONECTAR") } }
@Composable private fun Join(done: (String, String) -> Unit, back: () -> Unit) { var id by remember { mutableStateOf("") }; var pass by remember { mutableStateOf("") }; Title("Entrar em rede", back); OutlinedTextField(id, { id = it }, label = { Text("ID da rede") }, Modifier.fillMaxWidth()); OutlinedTextField(pass, { pass = it }, label = { Text("Código/senha") }, Modifier.fillMaxWidth()); Spacer(Modifier.height(18.dp)); Button({ done(id, pass) }, Modifier.fillMaxWidth()) { Text("CONECTAR") } }

@Composable private fun NetworkScreen(n: com.mikael.lan.data.LanNetwork?, vm: MainViewModel, back: () -> Unit, diag: () -> Unit) { Title(n?.name ?: "Rede", back); StatusCard("Conectado", "IP virtual: ${n?.virtualIp ?: "-"}", n?.peers?.size ?: 0); Spacer(Modifier.height(14.dp)); Text("Jogadores", style = MaterialTheme.typography.titleLarge); n?.peers?.let { LazyColumn(Modifier.weight(1f, false)) { items(it) { p -> Text("● ${p.name} — ${p.virtualIp} — ${p.pingMs ?: "?"} ms (${p.transport})", Modifier.padding(vertical = 8.dp)) } } }; Text("Conectar diretamente: ${n?.virtualIp ?: "10.10.0.x"}:25565", color = Color.LightGray); Spacer(Modifier.height(12.dp)); Button({ vm.runDiagnostics(); diag() }, Modifier.fillMaxWidth()) { Text("DIAGNÓSTICO") }; TextButton(back, Modifier.fillMaxWidth()) { Text("SAIR DA REDE") } }
